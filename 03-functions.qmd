# Block 3: Writing functions {.unnumbered}
...
the running example, turning a messy script into organised, tested code.*
...
The superassigment operator or `<<-` is used for **lexical scoping** it *will* modify the variable in a parent environment. If no such variable exists it will create one!
...
Write a function `fahr_to_celsius()` that converts Fahrenheit to Celsius. 

The conversion is `(fahr - 32) * 5 / 9`.
...
Arguments are matched first by exact name, then by position. This means
the order you write arguments in the call matters only when you rely on
position rather than naming them.
...
power(exponent = 3, base = 2)     # 8, matched by name, order irrelevant
power(2, exponent = 3)            # 8, mixed: name some, position the rest

Naming arguments at the call site makes code easier to read and protects
against errors when a function has several parameters. Relying on position
is acceptable only for the first one or two well-known arguments.
...
::::
::::{.task-container}
Using the `power()` function above, write three calls that all return 8
but use position matching, name matching, and a mix of the two.
::::
:::

### Wrapper functions and passing arguments with ...

A wrapper function calls another function, often to set sensible defaults
or simplify a common use. The `...` (dots) construct collects any extra
arguments and forwards them unchanged to the inner function. This lets a
wrapper expose the full flexibility of the function it wraps without
listing every argument.

```{r, eval = FALSE}

# A wrapper around mean() that removes missing values by default,
# but still forwards any other arguments the caller supplies.
mean_clean <- function(x, ...) {
  mean(x, na.rm = TRUE, ...)
}

v <- c(1, 2, 3, 4, 5, 6, 7, 8, 100, NA)
mean_clean(v)                # 2.333..., na.rm handled for you
mean_clean(v, trim = 0.25)   # trim passed through via ...
```
...
::::
::::{.task-container}
Write a function `round2()` that wraps `round()` and uses `digits = 2`
as its default, while still allowing the caller to override `digits` or
pass any other argument through with `...`.
::::
:::
...
```{r, eval = FALSE}
penguins_clean |> dplyr::filter(species == "Adelie") |> dplyr::summarise(n = dplyr::n())
```
...
penguins_test_subset <- penguins_clean |>
  tidyr::drop_na(body_mass_g, flipper_length_mm, species) |>
