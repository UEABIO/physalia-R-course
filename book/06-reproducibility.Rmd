...
```{r repro-table, echo=FALSE, message=FALSE}
library(tidyverse)
library(knitr)
library(kableExtra)
...
**Opening activity — why dont *you* share?** *(type your answer in the chat)*
...
> **Gomes, D.G.E., et al. (2022).** Why dont we share data and code? Perceived barriers and benefits to public archiving practices. *Proceedings of the Royal Society B*, 289, 20221113. https://doi.org/10.1098/rspb.2022.1113
...
```{r, eval=TRUE, echo=FALSE, out.width="80%", fig.cap = "The Many Analysts project (Gould et al 2023)"}
...
Report into the shared document (link in chat):
...
```{r many-analysts-demo, echo=FALSE, fig.cap="Results from the many-analysts activity. Each point is one participant's estimate for the daf-2 RNAi effect on C. elegans total offspring. Replace the simulated data below with actual collected results.", fig.height=3.5, fig.width=7}
...
```{r data-problems, echo=FALSE}
tribble(
  ~`#`, ~Problem,                                            ~Column,      ~Detail,                                                                                 ~`Why it matters`,
  "1",  "Mixed capitalisation in treatment",                  "`treatment`", "`daf` (n=117) and `DAF` (n=24) — same condition, will be treated as two groups",        "Inflates group count; DAF worms silently excluded or double-counted",
  "2",  "Trailing whitespace in treatment",                   "`treatment`", "`empty_vector ` (with trailing space, n=3) — will not match `empty_vector`",            "Three worms silently dropped or form a phantom third group",
  ...
  "8",  "Junk rows (501–506)",                                "All",         "Six rows where B, treatment, and diet are all NA — only TO has a value",               "Probably data entry artefacts; will cause errors in grouped summaries",
...
# C. elegans daf-2 × diet reproduction experiment
...
| treatment | character | — | | "DAF" = capitalisation error for "daf" |
