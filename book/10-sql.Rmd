...
microbenchmark( for (i in 1:100) {  # Perform 100 simulations for each sample size
    ...
    if ( coef(summary(lm_model))[2,4] < 0.05) {
...
The above works just fine - but we are likely to be working with a database when the file is too large to be held in the Global Environment in the first place. We need to use our programming skills to read our data in chunks and add to the datavase
...
process_and_insert <- function(skip_rows) {
  ...
  data_chunk <- fread(csv_name, skip = skip_rows, nrows = 500, showProgress = FALSE)
