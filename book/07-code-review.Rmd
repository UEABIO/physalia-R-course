...
```{r 4rs-table, echo=FALSE}
tribble(
  ~`#`, ~R,             ~Question,                        ~Tagline,
  "1",  "Reported",     "Is the code as reported?",       "Methods and code must match",
...
```{block, type="note"}
**A bridge from this morning.** `ANALYSIS.R` analyses the same experiment as the reproducibility chapter, but its author used different names: `strain` for what the data dictionary calls `treatment`, `block` for `B`, and `total_offspring` for `TO`. That drift between code and data dictionary is itself a small *Reported* problem — but to keep the review readable we use the script's own names below.
```
...
- Could the style be improved (e.g. would using Air or styler help, see Extra Material, intro to R)
...
tribble(
  ...
  "Reliable",     "Is the analysis sample selected with correct operator precedence?",
...
1. For each numbered section (1–7), name which problems it fixes. Two of the twelve are *not* fixed by this script — which two, and why? (Hint: both belong to the raw-data reshaping — the `pivot_longer` and the disk round-trip — that this script no longer does.)
...
Type your answers to questions 1 and 2 into the chat.
```
...
```{block, type="try"}
**Plan your group** *(~5 min — type in chat, or pair up if you're in the same room)*
...
3. **When and where** — cadence and platform (in-person, Zoom, async via GitHub PRs).
