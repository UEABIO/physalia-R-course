# Block 4: Iterations with purrr {.unnumbered}
...
```{r}
#| eval: false
vec_list <- list(a = 1:10, b = 11:20, c = 21:30)

purrr::map(vec_list, mean)   # a list of three means
```
...
```{r}
#| eval: false
purrr::map_dbl(vec_list, mean)   # a named numeric vector, not a list
```
...
## The base R alternative: a `for` loop

Before `map()`, the same job needs a loop with manual bookkeeping: pre-allocate
an output container, then assign into it by index.
...
means <- vector("double", length(vec_list))   # pre-allocate data type and length

names(means) <- names(vec_list)                # carry names across by hand

for (i in seq_along(vec_list)) {
  means[[i]] <- mean(vec_list[[i]])
}

means
```

`purrr::map_dbl(df_list, mean)` collapses all of this into one call. The loop is
not wrong, and understanding it is worth doing, but three jobs are now yours
rather than the function's: choosing the container type, sizing it, and preserving
names. Growing the vector instead of pre-allocating (`means <- c(means, ...)`
inside the loop) copies the whole object on every iteration, which turns linear
work into quadratic and is the classic loop performance trap. `map_dbl()` handles
the container, the sizing and the names, and fixes the return type. The honest
contrast is delegated bookkeeping, not raw speed: a correctly pre-allocated loop
and `map_dbl()` run in comparable time.
...
```{r}
#| eval: false
purrr::map_dbl(vec_list, \(x) sum(x) / length(x))
```
...
```{r}
#| eval: false
purrr::map_dbl(vec_list, function(x) sum(x) / length(x))
```
...
```{r}
#| eval: false
purrr::map_dbl(vec_list, \(x) mean(x, na.rm = TRUE))   # inside the lambda
purrr::map_dbl(vec_list, mean, na.rm = TRUE)           # forwarded by map()
```
...
# --- Setup: write the penguins data out as several CSV files ---
# In a real project these files would already exist; here we create them
# so the reading example has something to read. We split by island, so
# each file holds the penguins recorded at one location.

dir.create(here("data", "many_files"), recursive = TRUE, showWarnings = FALSE)

penguins_clean |>
  group_by(island) |>
  group_walk(\(rows, key) {
    write_csv(rows, here("data", "many_files", paste0(key$island, ".csv")))
  })

# --- Reading the files back in ---
files <- list.files(here("data", "many_files"),
                    pattern = "\\.csv$", full.names = TRUE)


# example to show list format before rbind
files |>
...
### `map2` and `pmap` and `imap`: iterating over more than one input
...
# pmap takes a list of vectors; each call receives one element from each.
pmap_dbl(list(sizes, widths, heights), \(s, w, h) s * w * h)  # 10 36 84
...
```{r}
# pmap walks any number of inputs in parallel. A data frame is just a named
# list of columns, so passing one lets each column become an argument.
# The function's parameter names should match the column names.
penguins_clean |>
  select(species, body_mass_g, flipper_length_mm) |>
  drop_na() |>
  slice_head(n = 3) |>
  pmap_chr(\(species, body_mass_g, flipper_length_mm) {
    glue::glue("{species}: Body weight {body_mass_g}g, Flipper length {flipper_length_mm}mm ")
  })
...
Note the slightly different approach to walk, here we split the data into a list and used `walk` in a previous example we used `group_walk`a variant designed explicitly to work on data after `group_by`
...
:::{.task}
::::{.task-header}
iterate over a different grouping
::::
::::{.task-container}
