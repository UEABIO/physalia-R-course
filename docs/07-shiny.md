...
<img src="images/skeleton_shiny.png" width="100%" style="display: block; margin: auto;" />
...
library(bslib) # Package for better looking apps
...
**UI Object:** Create an object called ui that defines the app's user interface. This specifies how the app will appear in the web browser. The `page_fluid()` function creates a responsive layout that adjusts to the browser window's size. All of your UI code will go inside the curly braces.
...
The easiest way to create a UI is to use one of the `page_*()` functions from {bslib}. The default Shiny app uses `fluidPage()` but a more modern version would be to use `page_fluid()` instead.
...
If you run this app now - it will load but present a blank screen -  we need to generate a front page and some server logic
...
Select from options with a check box: ` checkboxGroupInput`. 
...
In a Shiny app, outputs are dynamic components that display information to users based on their inputs. 

To create these outputs, you define them in the User Interface (UI) with specific IDs. 

You can add all kinds of different outputs to your UI with dedicated output functions. For example, let’s add a text output using `outputText()` function, a plot output with `plotOutput` and a table with `DT::dataTableOutput()`
...
<img src="07-shiny_files/figure-html/unnamed-chunk-37-1.png" width="100%" style="display: block; margin: auto;" />
...
The server is the brains behind the Shiny operation. This function makes sure that things actually happen in the app. In the Shiny framework, the server is basically nothing but a function with the arguments input, output, and session. As we’ve seen earlier, the smallest server you can think of is just an empty function.
...
Earlier, we gave the `textOutput()` the ID demo_text. Hence, we have to assign render instructions to `output$demo_text`. And to assign these instructions, there’s the `renderText()` function.

Similarly when we gave the `plotOutput()` the ID demo_plot, we therefore have to assing the render instructions to `output$demo_plot` and use the `renderPlot()` function
...
In this example I want the input of species to be reactive to user inputs, inside the function renderPlot this works just fine:
...
But if we try to change the filtering outside of a render function we get an error: 
...
We wrap all of the server functions that we want to run as part of a trigger inside `observeEvent()` and set the trigger in the UI.
